...
func TestReadFSMetadata(t *testing.T) {
	...
	if _, err := obj.PutObject(context.Background(), bucketName, objectName, mustGetPutObjReaders(t, bytes.NewReader([]byte("abcd")), int64(len("abcd")), "", ""), nil, ObjectOptions{}); err != nil {
		t.Fatal("Unexpected err: ", err)
	}
