...
func testAPIDeleteMultipleObjectsHandler(obj ObjectLayer, instanceType, bucketName string, apiRouter http.Handler,
	...
	for i := 0; i < 10; i++ {
		...
		_, err = obj.PutObject(context.Background(), bucketName, objectName, mustGetPutObjReaders(t, bytes.NewBuffer(contentBytes), int64(len(contentBytes)), "", sha256sum), nil, ObjectOptions{})
