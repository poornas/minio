...
func TestPutObjectPartFaultyDisk(t *testing.T) {
	...
	opts := ObjectOptions{}
	uploadID, err := fs.NewMultipartUpload(context.Background(), bucketName, objectName, map[string]string{"X-Amz-Meta-xid": "3f"}, opts)
	if err != nil {
		t.Fatal("Unexpected error ", err)
	}
	...
	_, err = fs.PutObjectPart(context.Background(), bucketName, objectName, uploadID, 1, mustGetHashReader(t, bytes.NewReader(data), dataLen, md5Hex, sha256sum), opts)
	if !isSameType(err, BucketNotFound{}) {
		t.Fatal("Unexpected error ", err)
	}
}
...
func TestCompleteMultipartUpload(t *testing.T) {
	...
	if _, err := fs.PutObjectPart(context.Background(), bucketName, objectName, uploadID, 1, mustGetHashReader(t, bytes.NewReader(data), 5, md5Hex, ""), opts); err != nil {
		t.Fatal("Unexpected error ", err)
	}
