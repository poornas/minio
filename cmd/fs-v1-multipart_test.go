...
func TestPutObjectPartFaultyDisk(t *testing.T) {
	...
	_, err = fs.PutObjectPart(context.Background(), bucketName, objectName, uploadID, 1, mustGetPutObjReaders(t, bytes.NewReader(data), dataLen, md5Hex, sha256sum), ObjectOptions{})
	if !isSameType(err, BucketNotFound{}) {
		t.Fatal("Unexpected error ", err)
	}
}
...
func TestCompleteMultipartUpload(t *testing.T) {
	...
	if _, err := fs.PutObjectPart(context.Background(), bucketName, objectName, uploadID, 1, mustGetPutObjReaders(t, bytes.NewReader(data), 5, md5Hex, ""), ObjectOptions{}); err != nil {
		t.Fatal("Unexpected error ", err)
	}
