...
func (l *s3EncObjects) GetObject(ctx context.Context, bucket string, key string, startOffset int64, length int64, writer io.Writer, etag string, o minio.ObjectOptions) error {
	...
	if err != nil {
		//fmt.Println(".................................................GO---------------", err, "no dmeta for ", key)
		// unencrypted content
		return l.s3Objects.GetObject(ctx, bucket, key, startOffset, length, writer, etag, o)
	}

	if len(dmeta.Parts) == 0 {
		//fmt.Println(".................................................GO---------------", err, "no parts for ", key)
	...
	for i, part := range dmeta.Parts {
		//fmt.Println(".................................................GO---------------", i, "<==part for ", part)
	...
	for i, part := range dmeta.Parts {
		...
		//fmt.Println("copied GONI... .WITH err ", err, " for part::: ", part.Number)
		if err != nil {
			logger.LogIf(ctx, err)
			return err
		}
...
// getPartmetadata fetches part.meta from s3 backend for the part and marshals into a structured format.
func (l *s3EncObjects) getPartMetadata(ctx context.Context, bucket, object, uploadID string, partID int, partETag string) (m gwMetaV1, err error) {
	partMetaFile := path.Join(object, defaultMinioGWPrefix, uploadID, strconv.Itoa(partID), partETag, gwpartMetaJSON)
	oi, err1 := l.s3Objects.GetObjectInfo(ctx, bucket, partMetaFile, minio.ObjectOptions{})
	if err1 != nil {
		return m, err1
	}
	var buffer bytes.Buffer
	err = l.s3Objects.GetObject(ctx, bucket, partMetaFile, 0, oi.Size, &buffer, oi.ETag, minio.ObjectOptions{})
	if err != nil {
		return m, err
	}
	return readGWMetadata(ctx, buffer)
	...
	/*
		var (
			...
			if dstOpts.ServerSideEncryption == nil || len(gwMeta.Parts) == 0 {
				//fmt.Println("should be here....", dstObject)
				return l.PutObject(ctx, dstBucket, dstObject, srcInfo.PutObjectReader, srcInfo.UserDefined, dstOpts)
			}
...
func (l *s3EncObjects) PutObject(ctx context.Context, bucket string, object string, data *minio.PutObjectReader, metadata map[string]string, opts minio.ObjectOptions) (objInfo minio.ObjectInfo, err error) {
	...
	if opts.ServerSideEncryption == nil {
		wasEncrypted := false
		if _, err := l.getDareMetadata(ctx, bucket, object); err == nil {
			wasEncrypted = true
		}
		fmt.Println("about to do backend put of ", object, " with opts nil")
		...
		fmt.Println("ETAG generated by PUT::", oi.ETag)
		return oi, nil
	}
	// overwrite any previous unencrypted object with same name
...
func (l *s3EncObjects) PutObjectPart(ctx context.Context, bucket string, object string, uploadID string, partID int, data *minio.PutObjectReader, opts minio.ObjectOptions) (pi minio.PartInfo, e error) {
	...
	fmt.Println(gwMeta.ETag, "<====encrypted ETag  ===> part ETag ", oi.ETag)
	if err = l.writeDareMetadata(ctx, bucket, path.Join(partUploadName, oi.ETag, gwpartMetaJSON), gwMeta, minio.ObjectOptions{}); err != nil {
		return pi, err
	}
...
func (l *s3EncObjects) CopyObjectPart(ctx context.Context, srcBucket, srcObject, destBucket, destObject, uploadID string,
	...
	// Get dare meta json
	_, err = l.getDareMetadata(ctx, destBucket, getTmpGWMetaPath(destObject, uploadID))
	if err != nil {
		return p, minio.InvalidUploadID{UploadID: uploadID}
	}
...
func (l *s3EncObjects) ListObjectParts(ctx context.Context, bucket string, object string, uploadID string, partNumberMarker int, maxParts int, opts minio.ObjectOptions) (lpi minio.ListPartsInfo, e error) {
	...
	for {
		...
		for _, obj := range loi.Objects {
			...
			var etag string
			// use encrypted etag if set
			if opts.GetEncryptedETagMetaFn != nil {
				etag = opts.GetEncryptedETagMetaFn(obj)
			}
			if partNum > 0 {
				pi := minio.PartInfo{
					PartNumber:   partNum,
