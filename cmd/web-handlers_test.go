...
func testDeleteBucketWebHandler(obj ObjectLayer, instanceType string, t TestErrHandler) {
	...
	for _, test := range testCases {
		if test.initWithObject {
			...
			_, err = obj.PutObject(context.Background(), test.bucketName, "object", mustGetPutObjReaders(t, data, int64(data.Len()), "", ""), nil, opts)
...
func testListObjectsWebHandler(obj ObjectLayer, instanceType string, t TestErrHandler) {
	...
	_, err = obj.PutObject(context.Background(), bucketName, objectName, mustGetPutObjReaders(t, bytes.NewReader(data), int64(len(data)), metadata["etag"], ""), metadata, ObjectOptions{})
...
func testDownloadWebHandler(obj ObjectLayer, instanceType string, t TestErrHandler) {
	...
	_, err = obj.PutObject(context.Background(), bucketName, objectName, mustGetPutObjReaders(t, bytes.NewReader(content), int64(len(content)), metadata["etag"], ""), metadata, ObjectOptions{})
	if err != nil {
		t.Fatalf("Was not able to upload an object, %v", err)
	}
...
func testWebHandlerDownloadZip(obj ObjectLayer, instanceType string, t TestErrHandler) {
	...
	obj.PutObject(context.Background(), bucket, "a/one", mustGetPutObjReaders(t, strings.NewReader(fileOne), int64(len(fileOne)), "", ""), nil, opts)
	obj.PutObject(context.Background(), bucket, "a/b/two", mustGetPutObjReaders(t, strings.NewReader(fileTwo), int64(len(fileTwo)), "", ""), nil, opts)
	obj.PutObject(context.Background(), bucket, "a/c/three", mustGetPutObjReaders(t, strings.NewReader(fileThree), int64(len(fileThree)), "", ""), nil, opts)
