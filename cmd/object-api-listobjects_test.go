...
func testListObjects(obj ObjectLayer, instanceType string, t TestErrHandler) {
	...
	for _, object := range testObjects {
		...
		_, err = obj.PutObject(context.Background(), testBuckets[0], object.name, mustGetPutObjReaders(t, bytes.NewBufferString(object.content),
...
func BenchmarkListObjects(b *testing.B) {
	...
	for i := 0; i < 20000; i++ {
		...
		_, err = obj.PutObject(context.Background(), bucket, key, mustGetPutObjReaders(b, bytes.NewBufferString(key), int64(len(key)), "", ""), nil, ObjectOptions{})
		if err != nil {
			b.Fatal(err)
		}
	}
