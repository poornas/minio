...
func runGetObjectBenchmark(b *testing.B, obj ObjectLayer, objSize int) {
	...
	for i := 0; i < 10; i++ {
		...
		objInfo, err = obj.PutObject(context.Background(), bucket, "object"+strconv.Itoa(i),
			mustGetHashReader(b, bytes.NewBuffer(textData), int64(len(textData)), md5hex, sha256hex), metadata, opts)
		if err != nil {
			b.Fatal(err)
		}
		if objInfo.ETag != md5hex {
	...
	for i := 0; i < b.N; i++ {
		...
		err = obj.GetObject(context.Background(), bucket, "object"+strconv.Itoa(i%10), 0, int64(objSize), buffer, "", opts)
		if err != nil {
			b.Error(err)
		}
	}
