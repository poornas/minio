...
func runPutObjectBenchmark(b *testing.B, obj ObjectLayer, objSize int) {
	...
	for i := 0; i < b.N; i++ {
		...
		objInfo, err := obj.PutObject(context.Background(), bucket, "object"+strconv.Itoa(i),
			mustGetPutObjReaders(b, bytes.NewBuffer(textData), int64(len(textData)), md5hex, sha256hex), metadata, ObjectOptions{})
		if err != nil {
			b.Fatal(err)
		}
	...
	for i := 0; i < b.N; i++ {
		...
		for j := 0; j < totalPartsNR; j++ {
			...
			partInfo, err = obj.PutObjectPart(context.Background(), bucket, object, uploadID, j,
				mustGetPutObjReaders(b, bytes.NewBuffer(textPartData), int64(len(textPartData)), md5hex, sha256hex), opts)
			if err != nil {
				b.Fatal(err)
			}
