...
func testXLReadStat(obj ObjectLayer, instanceType string, disks []string, t *testing.T) {
	...
	for i, input := range putObjectInputs {
		...
		_, err = obj.PutObject(context.Background(), input.bucketName, input.objectName, mustGetPutObjReaders(t, bytes.NewBuffer(input.textData), input.contentLength, input.metaData["etag"], ""), input.metaData, ObjectOptions{})
...
func testXLReadMetaParts(obj ObjectLayer, instanceType string, disks []string, t *testing.T) {
	...
	for _, testCase := range createPartCases {
		_, perr := obj.PutObjectPart(context.Background(), testCase.bucketName, testCase.objName, testCase.uploadID, testCase.PartID, mustGetPutObjReaders(t, bytes.NewBufferString(testCase.inputReaderData), testCase.intputDataSize, testCase.inputMd5, sha256sum), opts)
		if perr != nil {
