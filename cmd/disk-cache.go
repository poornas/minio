...
type cacheObjects struct {
	...
	PutObjectFn               func(ctx context.Context, bucket, object string, data *PutObjReaders, metadata map[string]string, opts ObjectOptions) (objInfo ObjectInfo, err error)
	...
	PutObjectPartFn           func(ctx context.Context, bucket, object, uploadID string, partID int, data *PutObjReaders, opts ObjectOptions) (info PartInfo, err error)
...
type CacheObjectLayer interface {
	...
	PutObject(ctx context.Context, bucket, object string, data *PutObjReaders, metadata map[string]string, opts ObjectOptions) (objInfo ObjectInfo, err error)
	...
	PutObjectPart(ctx context.Context, bucket, object, uploadID string, partID int, data *PutObjReaders, opts ObjectOptions) (info PartInfo, err error)
...
func (c cacheObjects) GetObjectNInfo(ctx context.Context, bucket, object string, rs *HTTPRangeSpec, h http.Header, lockType LockType, opts ObjectOptions) (gr *GetObjectReader, err error) {
	...
	go func() {
		opts := ObjectOptions{}
		putErr := dcache.Put(ctx, bucket, object, NewPutObjReaders(hashReader, nil, nil), c.getMetadata(bkReader.ObjInfo), opts)
...
func (c cacheObjects) GetObject(ctx context.Context, bucket, object string, startOffset int64, length int64, writer io.Writer, etag string, opts ObjectOptions) (err error) {
	...
	err = dcache.Put(ctx, bucket, object, NewPutObjReaders(hashReader, nil, nil), c.getMetadata(objInfo), opts)
	if err != nil {
		return err
	}
	pipeReader.Close()
...
func (c cacheObjects) PutObject(ctx context.Context, bucket, object string, r *PutObjReaders, metadata map[string]string, opts ObjectOptions) (objInfo ObjectInfo, err error) {
	...
	go func() {
		oinfo, perr := putObjectFn(ctx, bucket, object, NewPutObjReaders(hashReader, nil, nil), metadata, opts)
		if perr != nil {
	...
	go func() {
		if err = dcache.Put(ctx, bucket, object, NewPutObjReaders(cHashReader, nil, nil), metadata, opts); err != nil {
...
func (c cacheObjects) PutObjectPart(ctx context.Context, bucket, object, uploadID string, partID int, r *PutObjReaders, opts ObjectOptions) (info PartInfo, err error) {
	data := r.Reader
	...
	go func() {
		info, err = putObjectPartFn(ctx, bucket, object, uploadID, partID, NewPutObjReaders(hashReader, nil, nil), opts)
		if err != nil {
			close(pinfoCh)
	...
	go func() {
		if _, perr := dcache.PutObjectPart(ctx, bucket, object, uploadID, partID, NewPutObjReaders(cHashReader, nil, nil), opts); perr != nil {
			wPipe.CloseWithError(perr)
			return
		}
	}()
