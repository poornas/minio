...
func migrateData(ctx context.Context, c *diskCache, bucket, object, oldfile, destDir string, metadata map[string]string) error {
...
func migrateOldCache(ctx context.Context, c *diskCache) error {
	...
	for _, bucket := range buckets {
		...
		for _, oMeta := range objMetaPaths {
			...
			if err := migrateData(ctx, c, bucket, object, prevCachedPath, destdir, meta.Meta); err != nil {
				continue
			}
